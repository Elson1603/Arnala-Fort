document.addEventListener('DOMContentLoaded', function() {
    ...
    function getBotResponse(message) {
        ...
        return "I'm sorry, I don't have specific information about that. Please ask about timings, tickets, location, weather, how to reach, activities, food or history.";
    }
...
// Quiz Data and Functions (moved outside DOMContentLoaded)
const quizData = [
    {
...
function loadQuiz() {
    ...
    if (prevBtn) prevBtn.disabled = currentQuiz === 0;
    if (nextBtn) nextBtn.textContent = currentQuiz === quizData.length - 1 ? 'Finish' : 'Next';
}
    ...
    if (huntForm) {
        huntForm.addEventListener('submit', (e) => {
            ...
            if (answer === treasureHuntData[currentClue].answer) {
                ...
                if (progressBar) progressBar.style.width = `${huntProgress}%`;

                if (currentClue < treasureHuntData.length) {
                    const clueElement = document.querySelector('.clue');
                    if (clueElement) clueElement.textContent = treasureHuntData[currentClue].clue;
                    if (huntFeedback) {
...
function resetHunt() {
    ...
    if (clueElement) clueElement.textContent = treasureHuntData[0].clue;
    if (huntFeedback) huntFeedback.textContent = '';
}
